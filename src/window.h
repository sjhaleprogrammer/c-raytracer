...
void swapchainCreate(State *state) {
    // First check if physical device supports the surface
    VkBool32 supported = VK_FALSE;
    vkGetPhysicalDeviceSurfaceSupportKHR(
        state->context.physicalDevice, 
        state->context.queueFamily, 
        state->window.surface, 
        &supported
    );
    
    if (supported != VK_TRUE) {
        fprintf(stderr, "Error: Physical device doesn't support the surface\n");
        // Handle this error - perhaps try a different physical device
        return;
    }
    ...
    state->window.swapchain.imageExtent = extent;


    // Handle the swapchain creation with better error reporting
    ...
    if (result != VK_SUCCESS) {
        ...
        // Add debugging info
        fprintf(stderr, "Surface format: %d\n", surfaceFormat.format);
        fprintf(stderr, "Image extent: %dx%d\n", 
